...
#extracting features
features <-
      read.table(unz(temp,'UCI HAR Dataset/features.txt'), row.names = 1)
dim(features)
...
#writing final tidy dataset into tidy_data.txt
write.table(FINALtidyset, file = "tidy_data.txt", row.names = F)  
